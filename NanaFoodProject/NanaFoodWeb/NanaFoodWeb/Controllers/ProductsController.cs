...
namespace NanaFoodWeb.Controllers
{
    ...
    public class ProductsController(IProductRepo productRepo) : Controller
    {
        readonly IProductRepo _productRepo = productRepo;
        public IActionResult Index()
        {
            var response = _productRepo.GetAll(1, 10);
        ...
        [HttpPost("Edit/{id}")]
        public IActionResult Edit(ProductDto productDto, int id)
        {
            if (!ModelState.IsValid)
            {
                return View(productDto);
